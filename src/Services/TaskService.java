...
public class TaskService {
    ...
    private static final byte NMEMBER_DO_TASK_TOGETHER = 1; // Số người cùng mao Làm NV bang
...
//            msg.writer().writeUTF(player.playerTask.taskMain.name);
    ...
    // Kiểm tra người chơi đên NPC trả nhiệm vụ
        ...
        if (TaskService.gI().isCurrentTask(player, idTaskCustom)) {
            ...
            switch (idTaskCustom) {
                ...
                case ConstTask.TASK_2_0:
                    if (isCurrentTask(player, idTaskCustom)) {
                        Service.gI().sendThongBao(player, "Bạn đang có "
                                + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).count + "/"
                ...
                case ConstTask.TASK_4_1:
                    ...
                    break;
                //--------------------------------------------------------------
                case ConstTask.TASK_5_0: // Đạt 20k sức mạnh
                    break;
                case ConstTask.TASK_5_1: // Tiêu diệt 20 con %9
                    if (isCurrentTask(player, idTaskCustom)) {
                        Service.gI().sendThongBao(player, "Bạn đánh được "
                                + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).count + "/"
                                + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).maxCount
                    ...
                    break;
                case ConstTask.TASK_5_2:  // Quay về nhà báo cáo
                    npcSay(player, ConstTask.NPC_NHA,
                    ...
                    break;
                //--------------------------------------------------------------
                case ConstTask.TASK_6_0: // Đạt 40k sức mạnh
                    break;
                case ConstTask.TASK_6_1: // Tìm viên 7 sao
                    break;
                case ConstTask.TASK_6_2: // Quay về nhà báo cáo 
                    npcSay(player, ConstTask.NPC_NHA,
                ...
                case ConstTask.TASK_10_1:
                    if (isCurrentTask(player, idTaskCustom)) {
                        Service.gI().sendThongBao(player, "Bạn đánh được "
                                + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).count + "/"
                                + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).maxCount
                                + transformName(player, " %12"));
                    }
                    break;
                case ConstTask.TASK_10_2:
                    if (isCurrentTask(player, idTaskCustom)) {
                        Service.gI().sendThongBao(player, "Bạn đã tìm thấy truyện");
                    }
                    break;
                case ConstTask.TASK_10_3:
                    ...
                    try {
                        InventoryServiceNew.gI().subQuantityItemsBag(player, InventoryServiceNew.gI().findItemBag(player, 85), 1);
                    } catch (Exception ex) {
                    }
                    InventoryServiceNew.gI().sendItemBags(player);
                ...
                case ConstTask.TASK_15_0:
                    if (isCurrentTask(player, idTaskCustom)) {
                        if (player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).count % 5 == 0) {
                            Service.gI().sendThongBao(player, "Bạn đánh được "
                                    + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).count + "/"
                                    + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).maxCount
                                    + " Bulon");
                ...
                case ConstTask.TASK_32_0:
                    Service.gI().sendThongBao(player, "Đã đạt 40k sức mạnh");
                    break;
                case ConstTask.TASK_32_1:
                    break;
                case ConstTask.TASK_32_2:
                    npcSay(player, ConstTask.NPC_QUY_LAO,
                            "Con thực sự làm ta ngạc nhiên đấy\n");
                    break;
            }
            InventoryServiceNew.gI().sendItemBags(player);
            return true;
    ...
    public void paySideTask(Player player) {
        if (player.playerTask.sideTask.template != null) {
            if (player.playerTask.sideTask.isDone()) {
                int goldReward = 0;
                Item dayKimTuyen = ItemService.gI().createNewItem((short) 1479);
                switch (player.playerTask.sideTask.level) {
                    ...
                    case ConstTask.HARD:
                        ...
                        dayKimTuyen.quantity++;
                        break;
                    case ConstTask.VERY_HARD:
                        goldReward = ConstTask.GOLD_VERY_HARD;
                        dayKimTuyen.quantity++;
                        InventoryServiceNew.gI().addItemBag(player, dayKimTuyen);
                        break;
                    case ConstTask.HELL:
                        goldReward = ConstTask.GOLD_HELL;
                        dayKimTuyen.quantity += 2;
                        InventoryServiceNew.gI().addItemBag(player, dayKimTuyen);
                        break;
                }
                if (Util.isTrue(100, 100)) {
                    ...
                    Service.gI().sendThongBao(player, "Ngươi Nhận Được " + goldReward + " Vàng");
                }
                if (Util.isTrue(20, 100)) {
                    Service.gI().sendMoney(player);
                    player.inventory.ruby += goldReward / 5_000_000;
                    Service.gI().sendThongBao(player, "Ngươi Nhận Được " + (goldReward / 5000000) + " Hồng Ngọc");
                }
                Service.gI().sendMoney(player);
                player.playerTask.sideTask.reset();
            } else {
