...
public class ChangeMapService {
    ...
    /**
     * Mở tab chuyển map trong capsule
     */
    ...
    // Chuyen map vào khu trống
    public void changeMap(Player pl, int mapId, int x, int y, boolean Empty) {
        if (Empty == true) {
            int mapChange = -1;
            ...
            if (map != null) {
                ...
                if (mapChange > -1) {
                    changeMap(pl, null, mapId, mapChange, x, y, NON_SPACE_SHIP);
                } else {
                    Service.gI().sendThongBaoOK(pl, "Không còn khu trống!!");
                }
            } else {
                System.out.println("Không tìm thấy map có ID " + mapId);
            }
        }
    }
...
//        }
        ...
        if (zoneJoin != null) {
            switch (zoneJoin.map.mapId) {
                ...
                case 4:
                case 12:
                case 18://trạm tàu vũ trụ trái đất
