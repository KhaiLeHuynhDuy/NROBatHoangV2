...
//            }
    ...
    private boolean existItemInList(List<Item> list, int tempId) {
        for (int a = 0; a < list.size(); a++) {
            if (list.get(a).isNotNullItem() && list.get(a).template.id == tempId) {
                return true;
            }
        }
        return false;
        ...
        if (itemAdd.template.isUpToUp) {
            for (Item it : items) {
                ...
                if (itemAdd.template.id == 457
                        ...
                        || (itemAdd.template.id >= 1268 && itemAdd.template.id <= 1273)) {
                    it.quantity += itemAdd.quantity;
                    itemAdd.quantity = 0;
                    return true;
                }

                if (it.quantity < 1_000_000) {
                    int add = 1_000_000 - it.quantity;
                    if (itemAdd.quantity <= add) {
                        it.quantity += itemAdd.quantity;
                        itemAdd.quantity = 0;
                        return true;
                    } else {
                        it.quantity = 1_000_000;
